...
#URL ejemplo https://groq.cc.upv.es/cgi-bin/nooc-ice-lti-receiver-seguro.py?file=../B2-writing-correction-LTI.html
...
def ensure_session_dir():
    """Crea el directorio de sesiones y limpia archivos antiguos."""
    ...
    # Limpieza automática de sesiones expiradas (Garbage Collector)
    try:
        now = time.time()
...
def get_safe_redirect_url(params):
    """
    Construye la URL de destino permitiendo subir 1 nivel (../) 
    pero bloqueando intentos de fuga mayores.
    """
    host = os.environ.get('HTTP_HOST') or os.environ.get('SERVER_NAME', 'localhost')
    requested_path = params.get('file', REDIRECT_URL)
    
    # Normalizar para resolver ./ y ../
    normalized_path = os.path.normpath(requested_path)
    
    # Contar niveles de subida (..)
    up_levels = normalized_path.split(os.sep).count('..')
    
    # Seguridad: Bloqueamos si intenta subir más de 1 nivel
    if up_levels > 1:
        # Si hay sospecha, volvemos al nombre del archivo base sin rutas
        safe_filename = os.path.basename(REDIRECT_URL)
...
def main():
    ...
    # Página de estado si no hay parámetros
